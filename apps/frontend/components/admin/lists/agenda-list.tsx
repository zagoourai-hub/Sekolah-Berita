"use client";

import { AdminEmptyState } from "@/components/admin/admin-empty-state";
import { AdminListActions } from "@/components/admin/admin-list-actions";
import { AdminStatusBadge } from "@/components/admin/admin-status-badge";
import { adminUi } from "@/components/ui/class.-wrapper";
import type { Agenda } from "@/types/school";

type AgendaListProps = {
  agendaList: Agenda[];
  isLoading: boolean;
  isDeleting: boolean;
  onEdit: (item: Agenda) => void;
  onDelete: (id: string) => void;
};

export function AgendaList({
  agendaList,
  isLoading,
  isDeleting,
  onEdit,
  onDelete,
}: AgendaListProps) {
  return (
    <div className={`${adminUi.card} ${adminUi.cardPadding}`}>
      <div className={adminUi.cardHeader}>
        <div>
          <h2 className={adminUi.title}>Data Agenda</h2>
          <p className={adminUi.description}>Daftar agenda sekolah.</p>
        </div>
      </div>

      <AdminEmptyState
        isLoading={isLoading}
        isEmpty={agendaList.length === 0}
        loadingText="Memuat data agenda..."
        emptyText="Belum ada agenda."
      />

      <div className="space-y-3">
        {agendaList.map((item) => (
          <div key={item.id} className={adminUi.listItem}>
            <div className={adminUi.listRow}>
              <div className="min-w-0">
                <h3 className={adminUi.listTitle}>{item.title}</h3>
                <p className={adminUi.listSubtitle}>
                  {item.location || "Lokasi belum diisi"}
                </p>
              </div>

              <div className={adminUi.listMeta}>
                <AdminStatusBadge status={item.status} />

                <AdminListActions
                  isDeleting={isDeleting}
                  deleteMessage="Yakin ingin menghapus agenda ini?"
                  onEdit={() => onEdit(item)}
                  onDelete={() => onDelete(item.id)}
                />
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
